...
var changeIDETheme = function(theme){
	...
	if(activeIDE.length > 0){
		for(var o in activeIDE){
			if(typeof activeIDE[o].setTheme != "undefined"){
				activeIDE[o].setTheme("ace/theme/"+tm);
			}else if(typeof activeIDE[o].setOption != "undefined"){
				activeIDE[o].setOption("theme",tm);
			}
		}
	}
};
var IDEThemeStore = Ext.create('Ext.data.Store', {
	fields: ['theme'],
	autoLoad:true,
	proxy: {
		type: 'jsonp',
...
var tabSeq = 0;
var openIDE = function(node,nodeValue){
			...
			tabs.add({
				...
				listeners:{
					...
					afterrender:function(){
						...
						activeIDE[seq] = IDE.Create(tId+"-innerCt",{
							mode:ex,
							engine:IDE_ENGINE,
						...
						if(Sess.get("user_share") == "yes"){
							...
							RealBoard.on("responseIDEValue"+Sess.getId()+iId,function(deltas){
								lockIDE = true;
								if(deltas.length && activeIDE[seq]){
									activeIDE[seq].applyChanges(deltas);	
								}
								lockIDE = false;
							});
						}
		    ...
		    listeners:{
		    	afterrender:function(_container,eOpts){
					...
					if(openedIDE) {
						...
						if( openHistory.hasOwnProperty("length") ){
							for(var a in openHistory){
								openIDE(openHistory[a]);
							}
						}
					}
